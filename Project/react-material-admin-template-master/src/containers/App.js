...
class App extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.state = {
      navDrawerOpen: false, isNight: false, time: ''
    };
  }
  ...
  componentDidMount(){
    ...
    this.loadscriptJS('https://maps.googleapis.com/maps/api/js?key=AIzaSyAGVEe14FpAXvLYF-d0BOxannqcSZrrCFw&libraries=places&callback=createGoogleMap');
  ...
  createGoogleMap() {  
    var map = new window.google.maps.Map(window.document.getElementById("google-map"), {
...
//format: navigator.geolocation.getCurrentPosition(success[, error[, [options]])
      navigator.geolocation.getCurrentPosition(function(position) {//success : A callback function that takes a GeolocationPosition object as its sole input parameter
        var pos = {
  ...
  render() {
    ...
    return (
          ...
          <div  style={{
            backgroundColor: this.state.isNight ? 'black' : 'yellow', height: 1600
          }}>
      ...
      <div style={{width: 1800, height: 500, marginLeft: 50}}
            id="google-map"
          />
        <input style={{height:30, marginTop: 10}} id="pac-input" class="controls" type="text" placeholder="Search Box"/>
